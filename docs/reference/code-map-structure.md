...
- `prompts/`：仓库侧仅保留 XML prompt 默认模板分片；首次运行会复制到 `.alice/prompt/*.xml`，再按固定顺序组装为运行时 `.alice/prompt/prompt.xml`。
...
- `backend/alice/cli/`：当前默认 Python CLI / TUI bridge 入口；`bootstrap.py` 通过 `backend/alice/core/prompts.py` 补齐 `.alice/config.json`、`.alice/prompt/*.xml`、`.alice/prompt/prompt.xml`、`.alice/memory/*` 等运行时脚手架，并把配置装配为 orchestration / lifecycle / workflow；`main.py` 通过它收口 runtime logging 与启动流程。
- `backend/alice/domain/`：execution、llm、memory、skills 等业务逻辑。
- `backend/alice/domain/execution/executors/`：执行后端实现；当前默认 `container` harness 装配到 `docker_executor.py` / `DockerExecutionBackend`，同时存在 `local_process_executor.py` / `LocalProcessExecutionBackend` 作为单容器 runtime 内直接本地进程执行路径。
...
- `backend/alice/infrastructure/`：legacy bridge、gateway、docker、cache、logging；其中 `bridge/server.py` 是 legacy bridge 兼容薄壳，`legacy_compatibility_serializer.py` 是 canonical/internal event -> legacy message 的兼容投影边界，`gateway/` 提供 websocket session、replay、projector 与 interrupt/request transport，`docker/config.py` 维护容器镜像、挂载与 `.alice` / workspace 映射模型，`container_manager.py` 同时承载 `DockerExecutionBackend`，`stream_manager.py` 仅保留为 heuristic 兼容资产。
- `backend/alice/core/`：config、container、event_bus、interfaces、registry；`config/` 维护 `.alice/config.json -> Settings` 解析，`prompts.py` 集中管理 prompt 分片顺序、模板复制与运行时聚合，`registry/command_registry.py` 负责 execution harness 注册与装配，`logging/configure.py` 收口结构化日志 schema 与分类路由。
