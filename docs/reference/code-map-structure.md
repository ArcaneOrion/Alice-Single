...
- `backend/alice/application/runtime/`：phase-2 runtime 双载荷入口，定义 `RuntimeContext`、`RequestEnvelope`、`RequestMetadata`、`TimeProvider` 与 `RuntimeContextBuilder`；builder 同时负责 runtime 视图与 canonical model request envelope。
...
- `backend/alice/application/workflow/`：工作流编排；`base_workflow.py` 的 `WorkflowContext` 同时携带 `runtime_context` 与 `request_envelope`，`function_calling_orchestrator.py` 负责 structured tool call -> execution -> tool message 回注。
...
- `backend/alice/domain/llm/services/`：`chat_service.py` 同时负责 `<runtime_context>` prompt 投影与 `RequestEnvelope` 请求消息装配，`stream_service.py` 负责 runtime event 与 structured tool-calling 流式输出。
- `backend/alice/infrastructure/`：legacy bridge、docker、cache、logging；其中 `bridge/server.py` 是 legacy bridge 兼容薄壳，`legacy_compatibility_serializer.py` 是 canonical/internal event -> legacy message 的兼容投影边界并负责投影可观测性，`stream_manager.py` 仅保留为 heuristic 兼容资产，不再作为 bridge 包级正式导出。
...
- Runtime Context / Request Envelope 收敛：`backend/alice/application/runtime/`、`backend/alice/application/agent/agent.py`、`backend/alice/application/services/orchestration_service.py`、`backend/alice/application/workflow/base_workflow.py`、`backend/alice/domain/llm/services/chat_service.py`。
