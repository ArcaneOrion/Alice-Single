...
class OrchestrationService:
    ...
    def create_from_config(
        ...
        llm_provider = llm_registry.create_provider(
            provider_name,
            ProviderCreateOptions(
                api_key=api_key,
                base_url=base_url,
        ...
        memory_manager = MemoryManager(
            ...
            llm_provider=(llm_provider if isinstance(llm_provider, OpenAIProvider) else None),
            max_working_rounds=max_working_rounds,
            stm_days_to_keep=stm_days_to_keep,
        )

        harness_bundle = get_command_registry().create_harness(
            harness_name,
            project_root=project_root,
        )
        ...
        execution_service = ExecutionService(
            executor=harness_bundle.executor,
