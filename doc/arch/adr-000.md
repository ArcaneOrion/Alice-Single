...
- **Summary**: 前端 TUI 新增文字拖拽选择与系统剪贴板复制粘贴功能，覆盖聊天历史/思考侧边栏/输入框三个区域，通过 arboard crate 访问系统剪贴板
- **Pre-Validation**: cargo test: 1 passed, 0 failed, 4 ignored | cargo clippy: 0 errors, 0 warnings (PATH排除Nix rustc)
- **Post-Validation**: cargo test: 101 passed (100 lib + 1 doctest) | cargo clippy: 0 errors 0 warnings | cargo fmt: clean
## References

- **Related ADRs**: 本条为 ADR-000,无前置
- **Commits**: ca45d4f feat(frontend): TUI 文字拖拽选择与系统剪贴板复制粘贴 [ADR-000]
- **Specs**: (尚无 spec-vc 层)
- **External**:
